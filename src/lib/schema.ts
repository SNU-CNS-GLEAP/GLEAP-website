...
export const posts = pgTable(
  "posts",
  {
    ...
    // 공지/학술 소식/활동 소식 3분류 고정. `type`과 달리 값이 늘어나지 않는다는 전제라
    // enum 대신이지만 여전히 check 제약으로 고정 — 값 목록은 src/lib/post-sections.ts 참고.
    // 이 문자열을 바꾸면 그 파일의 POST_SECTIONS도 반드시 같이 갱신할 것.
    section: text("section").notNull().default("notice"),
  ...
  (table) => [check("posts_section_check", sql`${table.section} in ('notice', 'academic', 'activity')`)],
);
